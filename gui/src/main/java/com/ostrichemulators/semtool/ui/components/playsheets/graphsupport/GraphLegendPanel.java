...
public final class GraphLegendPanel extends JPanel implements GraphListener {
	...
	public void graphUpdated( DirectedGraph<SEMOSSVertex, SEMOSSEdge> graph, GraphPlaySheet gps ) {
		...
		MultiMap<IRI, SEMOSSVertex> types = new MultiMap<>();
		Map<IRI, NamedShape> shapes = new HashMap<>();
		Map<IRI, Color> colors = new HashMap<>();
		...
		for ( SEMOSSVertex v : vs ) {
			IRI type = SimpleValueFactory.getInstance().createIRI( v.getType().stringValue() );
			types.add( type, v );
		...
		for ( Map.Entry<IRI, List<SEMOSSVertex>> en : types.entrySet() ) {
