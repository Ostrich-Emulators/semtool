...
public class GridPlaySheet extends GridRAWPlaySheet {
	...
	private final LabeledPairTableCellRenderer<IRI> renderer = new LabeledPairTableCellRenderer<>();

	private final MultiSetMap<Integer, Integer> urilocations = new MultiSetMap<>();

	private final Map<RowCol, Resource> uris = new HashMap<>();
	/**
	 * The copy-paste mediator that listens for CTRL+C and CTRL+V keystrokes when
	...
	/**
	 * The Header Action Mediator is responsible for handling left-click and
	 * CTL+left-click events on a column header, initiating a column-select or a
	 * sort, respectively, to match a User's expectations in using MS Excel.
	 *
	 * @author Wayne Warren
	 *
	 */
	public class HeaderActionMediator extends MouseAdapter {
		...
		/**
		 * The current sorting direction of the clicked column, which enables
		 * toggling through simple logic.
		 */
		 ...
		 * Default constructor
		 *
		 * @param table The JTable to which this mediator will listen and act upon
		 */
		...
		public void mouseClicked( MouseEvent event ) {
			...
			if ( ( event.getModifiersEx() & ActionEvent.CTRL_MASK ) == ActionEvent.CTRL_MASK ) {
	...
	public class CopyPasteMediator implements ActionListener {
		...
		/**
		 * Default constructor, registers key events to the table as a side effect
		 ...
		 *
		 * @return True if the selection is valid and contiguous, and false
		 * otherwise
		 */
		...
		/**
		 * Paste data into the JTable of the parent class, currently found in the
		 * system clipboard
		 *
