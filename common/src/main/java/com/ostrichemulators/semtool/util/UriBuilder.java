...
public class UriBuilder {
	...
	public UriBuilder copy() {
		...
		try {
			bldr.setSanitizer( sanitizer.getClass().getDeclaredConstructor().newInstance() );
		}
	...
	public static UriBuilder getBuilder( String ns ) {
		...
		UriSanitizer sani;
		try {
			ldr = bldrclass.getDeclaredConstructor().newInstance();
		}
		catch ( IllegalAccessException | IllegalArgumentException | InstantiationException | NoSuchMethodException | SecurityException | InvocationTargetException e ) {
		...
		try {
			sani = saniclass.getDeclaredConstructor().newInstance();
		}
		catch ( IllegalAccessException | IllegalArgumentException | InstantiationException | NoSuchMethodException | SecurityException | InvocationTargetException e ) {
	 ...
	 * {@link #contains(org.eclipse.rdf4j.model.Resource)} because the namespace
	 * must match the this builder's content exactly, and not just start the same
	 * way
	 *
	 * @param IRI the IRI to check
	 * @return true, if
