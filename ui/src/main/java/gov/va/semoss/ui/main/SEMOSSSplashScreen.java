...
public class SEMOSSSplashScreen extends Frame {
	...
	/** The height of the area reserved for a String displayed at the 
	 * bottom of the SplashScreen, indicating what is happening, or other info */
	...
	/**
	 * Default constructor
	 * @param imagePath The path of the internal resource image used for this splash screen
	 */
	public SEMOSSSplashScreen(String imagePath){
		...
		if (image == null ) {
			log.warn( "Could not find resource: " + imagePath + ", using a blank image for splash screen" );
		...
		if (imageWidth > currentScreenSize.width && imageHeight > currentScreenSize.height){
			splashScreenSize = new Dimension(currentScreenSize.width/2, currentScreenSize.height/2 + PROGRESS_GUTTER_HEIGHT + MESSAGE_GUTTER_HEIGHT);
		}
		// Otherwise, use the image size to establish the splash screen bounds
		else {
			splashScreenSize = new Dimension(imageWidth, imageHeight + PROGRESS_GUTTER_HEIGHT + MESSAGE_GUTTER_HEIGHT);
		}
		...
		displaySplashScreen(splashOriginX, splashOriginY);
	}
	 ...
	 */
	public void displaySplashScreen(int x, int y){
	...
	public void paintComponents(Graphics graphics){
		...
		if (renderingCycles < 2){
        	...
        	graphics.drawString("\u00A9 Distributed under the GNU General Public License", 100, PROGRESS_GUTTER_HEIGHT + splashScreenSize.height - MESSAGE_GUTTER_HEIGHT + 6);
		}
	...
	/**
	 * Utility method to derive the percentage of "loading"
	 * that is complete
	 ...
	 * @param complete The percent complete from 0.0 to 1.0
	 */
	public static void setPercentLoadingComplete(float complete){
