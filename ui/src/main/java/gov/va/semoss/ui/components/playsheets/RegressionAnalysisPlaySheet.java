...
import gov.va.semoss.ui.main.listener.impl.RegressionDepVarListener;
import gov.va.semoss.ui.main.listener.impl.RegressionIndepVarDeleteListener;
import gov.va.semoss.ui.main.listener.impl.RegressionIndepVarListener;
...
public class RegressionAnalysisPlaySheet extends AbstractRDFPlaySheet {
	...
	/**
	 * Method createView. Creates the UI components necessary for the regression
	 * analysis play sheet and runs the initial query to pull all the properties
	 * that regression can be performed on.
	 */
	@Override
	public void createView() {
		...
		entityFillerSC = new EntityFillerForSubClass( getEngine(), boxes, "Concept" );
		entityFillerSC.run();
	}
	...
	public void resetInputList() {
		...
		if ( engine != null && rs == null ) {
			...
			String query = "SELECT DISTINCT ?property WHERE{{?node <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <" + nodeType + "> ;} {?node ?property ?Value}{?property <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Relation/Contains> ;}} ORDER BY ?property";
			...
			try {
				...
				progressComplete( "An error has occurred. Please check the query." );
				return;
			}
		...
		progressComplete( "100%...Table Generation Complete" );
	}
	...
	/**
	 * Method addPanel. Creates the main panel that will hold the list of
	 * properties to choose from and the dependent variables and regressors
	 * selected.
	 */
	public void addPanel() {
		...
		try {
			...
			log.debug( "Added the internal frame listener " );
			...
			nodeSelectorCombo = new ParamComboBox();
			...
			selDepVarBtn = new JButton( "Select Dependent Variable" );
			...
			selIndVarBtn = new JButton( "Select Regressors" );
			...
			deselIndVarBtn = new JButton( "Deselect Regressors" );
			...
			runRegressionAnalysisBtn = new JButton( "Run" );
	...
	/**
	 * Method addToMainPane. Adds the pane to the JInternalFrame.
	 *
	...
	protected void addToMainPane( JComponent pane ) {
		...
		log.info( "Adding Main Panel Complete" );
	}

	/**
	 * Method showAll. Displays all the UI components.
	 */
	public void showAll() {
		this.pack();
		this.setVisible( true );
