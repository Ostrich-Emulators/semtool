...
public class SesameJenaConstructWrapper {
	...
	private static final Logger logger = Logger.getLogger(SesameJenaConstructWrapper.class);
	...
	public SesameJenaConstructStatement next()
	{
		...
		try
		{
			if(engineType == IEngine.ENGINE_TYPE.SESAME)
			{
				...
				logger.debug("Adding a sesame statement "+stmt);
			...
			else
			{
				...
				logger.debug("Adding a JENA statement "+stmt);
				curSt = stmt;
