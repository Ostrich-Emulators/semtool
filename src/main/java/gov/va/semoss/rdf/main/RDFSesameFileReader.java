...
public class RDFSesameFileReader {
  private static final Logger log = Logger.getLogger(RDFSesameFileReader.class);
	...
	public static void main(String [] args) throws Exception
	{
		...
		while(res.hasNext())
		{
			...
			log.debug(">>> " + stmt.getSubject()+ " <> " + stmt.getPredicate() 
          + "<>" + stmt.getObject());
		}	
...
{?subject <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/System>.}
		...
		while(sparqlResults.hasNext())
		{
			...
			log.debug(" DOMAIN >>> " + bs.getBinding("domain") + "  " + bs.getBinding("relation") + "  Range >>> " + bs.getBinding("range")); // + "<<>>" + bs.getBinding("predicate")+"<<>>" + bs.getBinding("object"));
		}
