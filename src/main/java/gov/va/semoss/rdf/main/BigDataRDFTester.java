...
import gov.va.semoss.om.DBCMVertex;
...
public class BigDataRDFTester {
	...
	private static final Logger logger = Logger.getLogger(BigDataRDFTester.class);
...
//    SPARQLResultsJSONWriter w = new SPARQLResultsJSONWriter(baos);
//    logger.debug( baos.toString() );
	...
	public void readDataAsRDF() throws Exception {
		...
		/*
		 * logger.debug("get statements: http://pk.com#entity ?p ?o ?g"); results =
...
//	 		"{?system2 <" + RDFS.LABEL + "> \"DEERS\";}"+
  ...
  public void execInsertQuery( String query ) throws SailException, UpdateExecutionException {
    ...
    catch ( MalformedQueryException | RepositoryException | UpdateExecutionException | SailException e ) {
      try {
        rc.rollback();
      }
      catch ( Exception e2 ) {
