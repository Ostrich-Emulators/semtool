...
public class RDFTester {
	...
	private static final Logger logger = Logger.getLogger(RDFTester.class);
...
public void doSPARQL() throws Exception
	...
	public void readDataAsRDF() throws Exception
	{
		...
		logger.debug("get statements: http://pk.com#knows ?p ?o ?g");
		results = sc.getStatements(vf.createURI("http://pk.com#knows2"), null, null, true);
		while(results.hasNext()) {
			logger.debug(results.next());
		}		

		logger.debug("get statements: http://pk.com#1 ?p ?o ?g");
		...
		logger.debug("get statements: http://pk.com#entity ?p ?o ?g");
