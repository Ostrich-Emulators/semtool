 ...
 * DistanceDownstreamProcessor.
 */
public class DistanceDownstreamInserter {

	Hashtable masterHash = new Hashtable();
	private static final Logger logger = Logger.getLogger( DistanceDownstreamInserter.class );
	...
	/**
	 * Uses the engine to get all data objects in the selected database. For each
	 * data object, creates the forest (system network query) and passes it to
	 * DistanceDownstreamProcessor Gets back the master hash from
	 * DistanceDownstreamProcessor so that it knows distances for the systems Uses
	 * the returned master hash to create insert query for the database, runs
	 * query.
	 */
	public void insertAllDataDownstream() {
		Collection<String> dataObjectsArray = getObjects( "http://semoss.org/ontologies/DataObject" );
		...
		for ( String dataObjectString : dataObjectsArray ) {
			...
			DelegateForest<SEMOSSVertex, SEMOSSEdge> dataForest = new DelegateForest<>();
			try {
			...
			//need to make sure that creators first have the chance to go from data object directly
			logger.debug( "SET SELECTED ::::::::::::::::::::::::::::::::::::::::: "
					+ processor.addSelectedNode( dataObjectString, 0 ) );
			...
			while ( keyIt.hasNext() ) {
				...
				String sysURI = sysVertex.getProperty( Constants.URI_KEY ) + "";
	...
	/**
	 * Gets network value from a hashtable given the data object key Iterates
	 * through the datahash and normalizes the network weights by dividing by the
	 * max credit value Replaces normalized values from the data hash into the
	 * network value hash, which has the data object strings as keys
	 *
	 ...
	 *
	 * @param vertHash Hashtable	Contains distance and path with the key being the
	 * actual vertex.
	...
	private double calculateWeights( Hashtable vertHash, String systemName, String dataName, int distance, String pathString, double maxCreditValue ) {
		...
		if ( soaValueHash.containsKey( dataName ) ) {
			dataSOAHash = (Hashtable) soaValueHash.get( dataName );
		}
		...
		if ( distance > 0 ) {
			...
			if ( networkValueHash.containsKey( dataName ) ) {
				dataNetHash = (Hashtable) networkValueHash.get( dataName );
			}
			...
			for ( int i = 0; i < distance; i++ ) {
				...
				double addedValue;
				if ( i == 0 ) {
					addedValue = difference * appreciationRate;
				}
				else {
					addedValue = prevValue * ( 1 - appreciationRate );
				}
				double oldValue = 0.0;
	 ...
	 *
	 * @return DelegateForest<DBCMVertex,DBCMEdge>	Forest, comprised of vertices
	 * and edges.
	 */
	public DelegateForest<SEMOSSVertex, SEMOSSEdge> createForest( String query ) throws Exception {
		...
		while ( sjw.hasNext() ) {
			...
			if ( !sct.getPredicate().contains( vert1.getProperty( Constants.VERTEX_NAME ) + "" ) && !sct.getPredicate().contains( vert2.getProperty( Constants.VERTEX_NAME ) + "" ) ) {
			...
			if ( edge == null ) {
				 ...
				 log.error("Predicate plugged is " + predicateName);
	...
	/**
	 * Gets all objects of a certain type - in this case, probably DataObject in
	 * order to populate the forest queries.
	 *
	 * @param type String	Must be integer or double
	 * @return string names of all the data objects.
	 */
	public Collection<String> getObjects( String type ) {
		EntityFiller filler = new EntityFiller();
		List<String> list = new ArrayList<>();
		for ( URI u : filler.getData( engine, type ) ) {
			list.add( u.stringValue() );
		}
		return list;
	}
}
