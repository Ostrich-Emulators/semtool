...
public class ImportDataListener implements IChakraListener {
	...
	public void actionPerformed(ActionEvent e) {
		// get the import data
		// trigger the import
		JComboBox<String> comboBox = (JComboBox<String>) DIHelper.getInstance().getLocalProp(Constants.IMPORT_COMBOBOX);
		JComboBox<String> typeBox = (JComboBox<String>)DIHelper.getInstance().getLocalProp(Constants.IMPORT_TYPE_COMBOBOX);
		
		ImportDataProcessor.IMPORT_METHOD importMethod = null;
		ImportDataProcessor.IMPORT_TYPE importType = null;
		ImportDataProcessor processor 
        = new ImportDataProcessor(DIHelper.getInstance().getProperty(Constants.BASE_FOLDER));
				
		String selection = comboBox.getSelectedItem() + "";
    switch ( selection ) {
      case "Add to existing database engine":
        importMethod = ImportDataProcessor.IMPORT_METHOD.ADD_TO_EXISTING;
        break;
      case "Modify/Replace data in existing engine":
        importMethod = ImportDataProcessor.IMPORT_METHOD.OVERRIDE;
        break;
      case "Create new database engine":
        importMethod = ImportDataProcessor.IMPORT_METHOD.CREATE_NEW;
        break;
      case "Create new RDBMS connection":
    ...
    switch ( typeSelection ) {
      case "CSV format":
        ...
        break;
      case "Microsoft Excel format":
        importType = ImportDataProcessor.IMPORT_TYPE.EXCEL;
        break;
      case "Natural Language Proessing":
		...
		processor.setFilesToImport( fileNames );
    List<File> toload = new ArrayList<>();
    for( String s : fileNames.split( ";" ) ){
      toload.add( new File( s.trim() ) );
    }
    ...
    IEngine engine = DIHelper.getInstance().getRdfEngine();
		...
		if(importMethod == ImportDataProcessor.IMPORT_METHOD.OVERRIDE){
			...
			if(proceedWithImport){
				successfulImport = processor.processOverride(importType, customBaseURI, engine);
			}
		}
        ...
        try{
					...
					el.loadToEngine( toload, engine, proceedWithImport, null );
					el.release();
          ...
          logger.error( ex, ex );
			...
			processor.setFilesToImport( fileNames );
      File enginedir = new File( processor.getBaseDir(), dbName );
      try{

        File smss = EngineUtil.createNew( enginedir, dbName, customBaseURI, 
            dbPropFile, mapFile, questionFile, toload, true, true, true, null );
        ...
        successfulImport = false;
			...
			String dbName = ((JTextField)DIHelper.getInstance().getLocalProp(Constants.DB_NAME_FIELD)).getText();
      successfulImport = processor.processNewRDBMS(customBaseURI, dbName, dbType, dbImportURL, dbImportUsername, dbImportPW);
		}
