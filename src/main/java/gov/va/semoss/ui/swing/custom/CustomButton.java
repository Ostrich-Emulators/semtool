...
public class CustomButton extends JButton {
	...
	public static final StyleProcessor PROCESSOR = new StyleProcessor() {
		@Override
		public void process( Object target, StyleRuleSet rs ) {
			if ( target instanceof CustomButton ) {
				CustomButton t = (CustomButton) target;
				if ( rs.contains( AruiRules.FOREGROUND_MOUSEDOWN ) ) {
					t.foregroundMouseDown = SwingUtils.asColor( rs.getParams( AruiRules.FOREGROUND_MOUSEDOWN ), 0 );
				}
				if ( rs.contains( AruiRules.FOREGROUND_MOUSEOVER ) ) {
					t.foregroundMouseOver = SwingUtils.asColor( rs.getParams( AruiRules.FOREGROUND_MOUSEOVER ), 0 );
				}
				if ( rs.contains( AruiRules.STROKE ) ) {
					t.stroke = SwingUtils.asColor( rs.getParams( AruiRules.STROKE ), 0 );
				}
				if ( rs.contains( AruiRules.STROKE_MOUSEDOWN ) ) {
					t.strokeMouseDown = SwingUtils.asColor( rs.getParams( AruiRules.STROKE_MOUSEDOWN ), 0 );
				}
				if ( rs.contains( AruiRules.STROKE_MOUSEOVER ) ) {
					t.strokeMouseOver = SwingUtils.asColor( rs.getParams( AruiRules.STROKE_MOUSEOVER ), 0 );
				}
				if ( rs.contains( AruiRules.FILL ) ) {
					t.fill = SwingUtils.asPaint( rs.getParams( AruiRules.FILL ), 0 );
				}
				if ( rs.contains( AruiRules.FILL_MOUSEDOWN ) ) {
					t.fillMouseDown = SwingUtils.asPaint( rs.getParams( AruiRules.FILL_MOUSEDOWN ), 0 );
				}
				if ( rs.contains( AruiRules.FILL_MOUSEOVER ) ) {
					t.fillMouseOver = SwingUtils.asPaint( rs.getParams( AruiRules.FILL_MOUSEOVER ), 0 );
				}
				if ( rs.contains( AruiRules.CORNERRADIUS ) ) {
					t.cornerRadius = rs.asInteger( AruiRules.CORNERRADIUS, 0 );
				}
				t.reload();
			}
		}
	...
	public CustomButton( Action a ) {
		...
		add( label, BorderLayout.CENTER );
		addMouseListener( mouseAdapter );
		label.setText( a.getValue( Action.NAME ).toString() );
		reload();
	}
